...
找不到游戏窗口时，它会把当前所有可见窗口的标题列出来 —— 中文客户端、改过标题、或装了 Reloaded-II 之类加载器的情况，直接用 `--title "其中一段"` 指定即可。跑完还会问一句要不要做虚拟手柄测试。

报告内容是英文的：非中文版 Windows 的控制台代码页（cp437/cp1252）编不了中文，`print` 中文会直接抛异常。
